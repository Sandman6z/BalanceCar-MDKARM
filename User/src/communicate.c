...
void Parse(char *dataInput)
{
	...
	if(Uart1Index){
		DebugOutBuff(Uart1Buffer, Uart1Index); //KKDD UART3->UART1
		DebugOutStr("\r\n");
	}
#endif
	while(((p-dataInput)<Uart1Index))//KKDD UART3->UART1
	{
		if(*p == '#')// 
		{
			//KKDD
			if(g_CarRunningMode == CONTROL_MODE)
					{
						if(strstr(p , "KKCS")){Steer(0, 0);}//停止
						else if(strstr(p, "KKCJ")){// 摇杆控制
							p +=4;
							strtok(p, ",");
	...
	Uart1Index=0; //KKDD UART3->UART1
	memset(Uart1Buffer, 0, Uart1BufferSize); //KKDD UART3->UART1
}
...
/*KKDD
	...
	如:#17,CM,-27.314,-5.716*56;  帧头# 长度17  内容CM,-27.314,-5.716 校验56，长度为内容字节数
KKDD*/
unsigned char ParseAT(char *dataInput)
{
	unsigned char len,check,ret;
	char *p = dataInput;
	float fTemp1, fTemp2;
